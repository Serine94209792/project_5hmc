...
def parse_feature_list(feature_list):
    """
    解析特征列表，提取特征类型和类别信息（支持 _proba_0, _proba_1, _proba_2, ...）
    """
    import re
    feature_info = []
    for idx, feat_name in enumerate(feature_list):
        m = re.match(r'^(.+)_proba_(\d+)$', feat_name)
        if m is None:
            raise ValueError(f"{feat_name} is not a valid feature name (expected {{type}}_proba_{{k}})")
        feat_type, class_label = m.group(1), m.group(2)
        feature_info.append({
            'index': idx,
...
def get_model_list(
    ...
    cv_matrix_dropped = cv_matrix.drop(columns=["test_index", "y_test"])
